...
export function ProductGrid() {
  ...
  return (
    <section id="shop" className="py-24 md:py-40 px-6 md:px-12 bg-background relative z-20">
      <div className="max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-24 gap-6">
          <div>
            <TextReveal className="font-serif text-5xl md:text-7xl uppercase tracking-widest text-textMain mb-4">
              The Archive
            </TextReveal>
            <p className="font-sans text-secondary max-w-md tracking-wider">
              Explore our complete collection of raw, washed, and distressed denim artifacts.
            </p>
          </div>
          <div className="flex gap-4">
            <button className="font-sans text-xs uppercase tracking-widest text-accent border-b border-accent pb-1 hover:text-white hover:border-white transition-colors">
              View All
            </button>
            <button className="font-sans text-xs uppercase tracking-widest text-secondary hover:text-white transition-colors">
              Filter
            </button>
          </div>
        </div>
          ...
          {products.map((product, index) => (
            ...
            >
              <div className="relative aspect-[3/4] overflow-hidden bg-primary mb-6">
                {product.tag && (
                  <div className="absolute top-4 left-4 z-20 bg-accent text-primary text-[10px] font-bold px-3 py-1 tracking-widest uppercase">
                    {product.tag}
                  </div>
                )}
                ...
                <div className="absolute bottom-4 left-0 w-full flex justify-center gap-4 opacity-100 translate-y-0 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-500 md:translate-y-4 md:group-hover:translate-y-0">
                  <button 
                  ...
                  <button className="w-12 h-12 bg-primary/80 backdrop-blur text-white flex items-center justify-center rounded-full hover:scale-110 transition-transform shadow-xl">
                    <Eye size={20} />
                  </button>
                  <button className="w-12 h-12 bg-primary/80 backdrop-blur text-white flex items-center justify-center rounded-full hover:scale-110 transition-transform shadow-xl">
                    <Heart size={20} />
              ...
              <div className="flex justify-between items-start gap-4">
                <div>
                  <h3 className="font-sans font-semibold text-sm tracking-widest uppercase text-textMain mb-1">
                    {product.name}
                  </h3>
                ...
                <span className="font-serif text-lg text-textMain">
                  {product.price}
                </span>
              </div>
            </motion.div>
